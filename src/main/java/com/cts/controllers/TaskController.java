...
public class TaskController {
	...
	@RequestMapping(value="/projectTaskUserManagement")
	public ModelAndView getInsurance(ModelAndView model){
		model.setViewName("projectTaskUserManagement");
		return model;
	}
	...
	@ResponseBody
	public ResponseEntity<List<Task>> addUpdateTask(@RequestBody Task task) 
			throws Exception{
		if(task.getTaskId() == 0){
			trepo.addTask(task);
		}else{
